...
class ReminderRow extends StatelessWidget {
  const ReminderRow({Key? key, required this.reminder, required this.onToggle})
      : super(key: key);
...
class EmptyRemindersState extends StatelessWidget {
  const EmptyRemindersState({Key? key, required this.filter}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    final title =
        filter == ReminderFilter.today ? 'На сегодня пусто' : 'Список пуст';
